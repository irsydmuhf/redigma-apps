...
export default async function LmsLoginPage({
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#07569d] px-4 py-10">
      <div className="w-full max-w-md space-y-8 rounded-3xl bg-white p-10 shadow-xl">
        <div className="space-y-3 text-center">
          ...
          />
          <p className="pt-1 text-sm font-medium text-neutral-700">ADV Onboarding</p>
          <p className="text-xs text-neutral-500">Login dengan email &amp; password</p>
        </div>

        {registered && (
          <div className="rounded-2xl bg-green-50 px-4 py-3 text-sm text-green-700">
            Akun berhasil dibuat! Silakan login — akses konten belajar akan aktif
            setelah Manager menyetujui pendaftaran Anda.
          </div>
        )}
        ...
        <form action={lmsLogin} className="space-y-5">
          ...
          <button
            type="submit"
            className="h-12 w-full rounded-2xl bg-[#07569d] text-sm font-semibold text-white shadow-md transition hover:bg-[#064074] active:translate-y-px"
          >
            Login
          </button>
        </form>
        ...
        <p className="text-center text-xs text-neutral-500">
          Belum punya akun?{" "}
          <span className="font-medium text-neutral-700">Minta invite link dari Manager Anda.</span>
        </p>
