-- =====================================================================
-- Phase 9: RBAC hardening + Direksi read-only
-- =====================================================================
-- Matriks akses target untuk tabel dinamis:
--                SELECT    INSERT    UPDATE    DELETE
-- admin            ✓         ✓        ✓         ✓
-- staff/spv/head   ✓         ✓        ✓         ✓  (divisi sendiri)
-- direksi          ✓         ✗        ✗         ✗  (semua divisi, read-only)
-- staff lain       ✗         ✗        ✗         ✗
--
-- Direksi & user lain di-deny via tidak adanya policy yang allow.
-- Operasi soft-delete & rollback tetap jalan via RPC SECURITY DEFINER.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. Update create_dynamic_table — tambah UPDATE/DELETE policies
-- ---------------------------------------------------------------------
...
begin
  ...
  -- SELECT: admin, direksi (read-only), member divisi (kalau row tidak soft-deleted)
  execute format(
  ...
  -- INSERT: admin atau staff/spv/head di divisi (direksi DENY)
  execute format(
  ...
  -- UPDATE: admin atau staff/spv/head di divisi (direksi DENY)
  execute format(
    'create policy dataset_update on public.%I for update to authenticated using (' ||
      'public.is_admin(auth.uid()) or ' ||
      'exists (select 1 from public.user_divisions where user_id = auth.uid() and ' ||
        'division_code = %L and role in (''staff'', ''spv'', ''head''))' ||
    ') with check (' ||
  ...
  -- DELETE: admin only (staff/spv/head harus soft-delete via rollback RPC)
  execute format(
    'create policy dataset_delete on public.%I for delete to authenticated using (' ||
...
-- ---------------------------------------------------------------------
-- 2. Add UPDATE/DELETE policies ke tabel dinamis YANG SUDAH ADA
-- ---------------------------------------------------------------------
do $$
declare
  v_ds record;
begin
  for v_ds in select physical_table_name, division_code from public.datasets
  loop
    -- Cek apakah policy update sudah ada
    if not exists (
      select 1 from pg_policies
      where schemaname = 'public'
        and tablename = v_ds.physical_table_name
        and policyname = 'dataset_update'
    ) then
      execute format(
        'create policy dataset_update on public.%I for update to authenticated using (' ||
          'public.is_admin(auth.uid()) or ' ||
